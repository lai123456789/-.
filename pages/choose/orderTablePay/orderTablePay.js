...
Page({
  ...
  onLoad: function (options) {
	...
	this.orderClassify = options.orderType
    if (options.store_n) {
	...
	this._menuConsoleInfo() //新增  获取菜单列表
    wx.login({
      success: (res) => {
        // 微信获取登录的code信息只能使用一次
        let code = res.code;
  ...
  _setReceiptTitle: function () {
    ...
    let receiptTitle = ''
    let orderClassify = this.orderClassify
  ...
  _prepaidOrder(callback) {
    ...
    prepaidOrder(param).then((res) => {
	  ...
	  let totalPreferential = res.amount.consumerAmount + res.amount.consumerAmountDiscount
	  let nameL = ['扫码收款','券付款','茶位费','配送费','打包费','最低消费']
	  let foodList = res.pro_array.filter((item,index) => {
	  	return nameL.indexOf(item.pro_name) == -1
	  })
	  let elseList = res.pro_array.filter(item => nameL.includes(item.pro_name))
      this.setData({
        money: ought_amount,
        practical_amount,
        consumer_amount: res.consumer_amount,
		confirmed_food_es: foodList,
		elseList,
		//
		consumerAmount: (res.amount.consumerAmount * 1).toFixed(2),
		consumerAmountDiscount: (res.amount.consumerAmountDiscount * 1).toFixed(2),
		totalPreferential: (totalPreferential * 1).toFixed(2)
      });
      if(callback) {
        callback()
      }
